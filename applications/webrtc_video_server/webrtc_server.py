...
def parse_ice_strings(ice_server_strings):
    ...
    logging.info(f"Parsed ice strings into {ice_server_list}")
...
class WebAppThread(Thread):
    def __init__(
        ...
        app.router.add_post("/iceSignaling", self._ice_signaling)
    ...
    async def _offer(self, request):
        ...
        (sdp, type) = await self._webrtc_server_op.handle_offer(
            params["sdp"], params["type"], params["peerid"]
        )
    ...
    # MZ test
    async def _ice_signaling(self, request):
        params = await request.json()
        peer_id = params["peerid"]
        await self._webrtc_server_op.add_ice_candidate(params["candidate"], peer_id)
...
if __name__ == "__main__":
    ...
    # --ice-server "turn:10.0.0.131:3478[admin:admin]"
    parser.add_argument(
        "--ice-server",
        action="append",
